            ...
            try
            {
                ...
                catch
                {
                    ...
                    using (NetworkStream outputStream = new NetworkStream(socket, false))
                    {
                        ...
                        byte[] bytes = Util.Serializer(m);
                        string name = m.GetType().Name;
                        if (m.GetType().IsGenericType)
                        {
                            int index = m.GetType().Name.IndexOf('`');
                            name = m.GetType().Name.Substring(0, index);
                        }
                        var buf = MsgType.GetBytes(name, bytes);
            ...
            lock (_nodeLock)
            {
                ...
                try
                {
                    ...
                    using (input = File.OpenRead(path))
                    {
                        ...
                        stableStorage = Util.Deserialize<NodeStableStorage<TValue>>(bytes);
            ...
            lock (_nodeLock)
            {
                ...
                try
                {
                    ...
                    using (outputStream = File.OpenWrite(path))
                    {
                        ...
                        byte[] bytes = Util.Serializer(stableStorage);
                        ...
                        try
                        {
                            using (socket = _serverSocket.Accept())
                            {
                                ...
                                byte[] bytes = new byte[1024];
                                 int num=  input.Read(bytes, 0, bytes.Length);
                                //  Message message=  Util.Deserialize<Msg>(bytes);
                                if(num==1024)
                                {
                                    //
                                    bytes= new byte[1024];
                                ...
                                Message message = MsgType.ConvertMsg(memory.Slice(0,num).ToArray());
                                tmp = message;
                                if (message is AcceptNotificationMessage<TValue>)
                                {
                                    AcceptNotificationMessage < TValue > acceptNotification=(AcceptNotificationMessage<TValue>)message;
                                    if (acceptNotification.Proposal == null)
                                    {
